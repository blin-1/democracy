...
/**
 * REST controller for managing {@link com.osi.democracy.domain.Address}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class AddressResource {
    ...
    public List<Address> getAllAddresses(@RequestParam(required = false) String filter) {
        if ("candidate-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where candidate is null");
            return StreamSupport
                .stream(addressRepository.findAll().spliterator(), false)
                .filter(address -> address.getCandidate() == null)
                .toList();
        }
